...
export const api = {
  ...
  uploadDocument: (fileName: string, projectId: number, category: DocumentCategory, uploaderId: number): Promise<Document> => {
    ...
    setTimeout(() => {
        const docIndex = db.documents.findIndex(d => d.id === newDoc.id);
        if (docIndex > -1) {
            db.documents[docIndex].status = DocumentStatus.SCANNING;
        }
    }, 1500);
    ...
    setTimeout(() => {
        const docIndex = db.documents.findIndex(d => d.id === newDoc.id);
        if (docIndex > -1) {
            // Simulate a random quarantine outcome for demonstration
            if (Math.random() > 0.9) {
                 db.documents[docIndex].status = DocumentStatus.QUARANTINED;
            } else {
                 db.documents[docIndex].status = DocumentStatus.APPROVED;
            }
        }
    }, 3500);
  ...
  assignEquipmentToProject: (equipmentId: number, projectId: number, actorId: number): Promise<Equipment> => {
      ...
      db.equipment[index].lastAssignedDate = new Date();
      db.equipment[index].maintenanceStartDate = null;
      db.equipment[index].estimatedAvailableDate = null;
  ...
  updateEquipmentStatus: (equipmentId: number, status: EquipmentStatus, actorId: number): Promise<Equipment> => {
      ...
      if (status === EquipmentStatus.AVAILABLE) {
          ...
          db.equipment[index].maintenanceStartDate = null;
          db.equipment[index].estimatedAvailableDate = null;
      } else if (status === EquipmentStatus.MAINTENANCE) {
          db.equipment[index].projectId = null;
          db.equipment[index].maintenanceStartDate = new Date();
          const availabilityDate = new Date();
          availabilityDate.setDate(availabilityDate.getDate() + 7); // Mock: available in 7 days
          db.equipment[index].estimatedAvailableDate = availabilityDate;
      }
  ...
  getDashboardActivityLogs: (companyId: number): Promise<AuditLog[]> => {
    const companyUserIds = new Set(db.users.filter(u => u.companyId === companyId).map(u => u.id));
    const recentLogs = db.auditLogs
        .filter(log => companyUserIds.has(log.actorId))
        .sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime())
        .slice(0, 10);
    return simulateNetwork(recentLogs);
  },
...
3.  **Long-term:** Consider revising work schedules to include a second short break in the mid-afternoon.
  ...
  // Principal Admin
  getPlatformUsageMetrics: (): Promise<UsageMetric[]> => {
    const activeProjects = db.projects.filter(p => p.status === 'Active').length;
    const thirtyDaysAgo = new Date().getTime() - (30 * 24 * 60 * 60 * 1000);
    const recentDocuments = db.documents.filter(d => new Date(d.uploadedAt).getTime() > thirtyDaysAgo).length;

    const metrics: UsageMetric[] = [
        { name: "Tasks Created (Last 30 Days)", value: Math.floor(Math.random() * 500) + 1200, unit: 'tasks' },
        { name: "Documents Uploaded (Last 30 Days)", value: recentDocuments, unit: 'files' },
        { name: "Active Projects", value: activeProjects, unit: 'projects' },
        { name: "API Calls (24h)", value: Math.floor(Math.random() * 5000) + 12000, unit: 'calls' }
    ];
