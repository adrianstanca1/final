...
const decodeGoogleCredential = (credential: string): GoogleCredentialPayload | null => {
    ...
    try {
        ...
        return JSON.parse(payload) as GoogleCredentialPayload;
    } catch (error) {
...
const isWithinLastDays = (value: unknown, days: number) => {
    ...
    return Date.now() - date.getTime() <= daysToMs(days);
};

const getCompanyUsers = (companyId: string) => db.users.filter(user => user.companyId === companyId);

const getCompanyProjects = (companyId: string) => db.projects.filter(project => project.companyId === companyId);

const getCompanyProjectIds = (companyId: string) =>
    new Set(getCompanyProjects(companyId).map(project => project.id).filter((id): id is string => Boolean(id)));

const getCompanyUserIds = (companyId: string) =>
    new Set(getCompanyUsers(companyId).map(user => user.id).filter((id): id is string => Boolean(id)));

const getTimeEntriesForCompany = (companyId: string) => {
    const projectIds = getCompanyProjectIds(companyId);
    ...
    return db.timeEntries.filter(entry => {
        ...
        if (entry.userId && userIds.has(entry.userId)) {
            return true;
        }
        return false;
    });
...
const getTodosForCompany = (companyId: string) => {
    const projectIds = getCompanyProjectIds(companyId);
    return db.todos.filter(todo => !todo.projectId || projectIds.has(todo.projectId));
};

const getDocumentsForCompany = (companyId: string) =>
    db.documents.filter(doc => {
        const resolvedCompany = resolveDocumentCompanyId(doc);
        return resolvedCompany === companyId;
    });

const getMessagesForCompany = (companyId: string) => {
    const userIds = getCompanyUserIds(companyId);
    return db.messages.filter(message => message.senderId && userIds.has(message.senderId));
};

const getInsightsForCompany = (companyId: string) => {
    const projectIds = getCompanyProjectIds(companyId);
    return db.projectInsights.filter(insight => insight.projectId && projectIds.has(insight.projectId));
};

const getAssignmentsForCompany = (companyId: string) =>
    db.resourceAssignments.filter(assignment => assignment.companyId === companyId);
...
type ToolDefinition = {
    ...
    resolver: (companyId: string) => { usageCount: number; lastUsedAt: string | null };
};

const adoptionFromRatio = (ratio: number): AdoptionLevel => {
    if (ratio >= 0.6) {
        return 'HIGH';
    }
    if (ratio >= 0.3) {
...
const TOOL_DEFINITIONS: ToolDefinition[] = [
    {
        id: 'timesheets',
        label: 'Timesheets',
        requiredPermissions: [Permission.SUBMIT_TIMESHEET, Permission.VIEW_ALL_TIMESHEETS, Permission.MANAGE_TIMESHEETS],
        views: ['time', 'timesheets'],
        recommendation: 'Encourage crews to submit weekly timesheets to keep payroll and compliance on track.',
        resolver: companyId => {
            const entries = getTimeEntriesForCompany(companyId);
            const recentEntries = entries.filter(entry =>
                isWithinLastDays(entry.endTime ?? entry.clockOut ?? entry.startTime ?? entry.clockIn, 30)
            );
            const latest = entries.reduce<Date | null>((acc, entry) => {
                const date = toDate(entry.endTime ?? entry.clockOut ?? entry.startTime ?? entry.clockIn);
                if (!date) {
                    return acc;
                }
            ...
            return {
                usageCount: recentEntries.length,
    ...
    },
    {
        ...
        requiredPermissions: [Permission.VIEW_DOCUMENTS, Permission.UPLOAD_DOCUMENTS],
        views: ['documents'],
        recommendation: 'Centralise drawings and approvals so teams are never working from stale files.',
        resolver: companyId => {
            const documents = getDocumentsForCompany(companyId);
            const recentDocs = documents.filter(doc => isWithinLastDays(doc.uploadedAt ?? doc.updatedAt, 30));
            const latest = documents.reduce<Date | null>((acc, doc) => {
                const date = toDate(doc.uploadedAt ?? doc.updatedAt ?? doc.createdAt);
                if (!date) {
                    return acc;
                }
                return !acc || date > acc ? date : acc;
    ...
    },
    {
        ...
        views: ['tools'],
        recommendation: 'Promote AI-guided insights to surface risks and automate reporting.',
        resolver: companyId => {
            const insights = getInsightsForCompany(companyId);
            const recentInsights = insights.filter(insight => isWithinLastDays(insight.updatedAt ?? insight.createdAt, 30));
            const latest = insights.reduce<Date | null>((acc, insight) => {
                const date = toDate(insight.updatedAt ?? insight.createdAt);
                if (!date) {
                    return acc;
                }
                return !acc || date > acc ? date : acc;
            ...
            return {
                usageCount: recentInsights.length,
                lastUsedAt: latest ? latest.toISOString() : null,
            };
        },
    },
    {
        ...
        views: ['chat'],
        recommendation: 'Adopt structured chat channels for faster field-to-office collaboration.',
        resolver: companyId => {
            const messages = getMessagesForCompany(companyId);
            const recentMessages = messages.filter(message => isWithinLastDays(message.timestamp, 30));
            const latest = messages.reduce<Date | null>((acc, message) => {
            ...
            return {
                usageCount: recentMessages.length,
                lastUsedAt: latest ? latest.toISOString() : null,
            };
        },
    },
    {
        ...
        requiredPermissions: [Permission.MANAGE_TEAM, Permission.MANAGE_EQUIPMENT],
        views: ['tools', 'equipment'],
        recommendation: 'Schedule labour and equipment allocations ahead of time to avoid conflicts.',
        resolver: companyId => {
            const assignments = getAssignmentsForCompany(companyId);
            const recentAssignments = assignments.filter(assignment =>
                isWithinLastDays(assignment.startDate ?? assignment.endDate, 30)
            );
            const latest = assignments.reduce<Date | null>((acc, assignment) => {
                const date = toDate(assignment.endDate ?? assignment.startDate);
                if (!date) {
                    return acc;
                }
                return !acc || date > acc ? date : acc;
            ...
            return {
                usageCount: recentAssignments.length,
                lastUsedAt: latest ? latest.toISOString() : null,
            };
        },
    },
...
export const authApi = {
    ...
    loginWithGoogle: async (credential: string): Promise<any> => {
        ...
        if (!user) {
            ...
            const createdAt = new Date().toISOString();
            const companyId = `google-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
            const companyName = payload.hd
                ? `${payload.hd} Workspace`
                : `${[firstName, lastName].filter(Boolean).join(' ') || 'New Company'} Workspace`;
            ...
            const resolvedFirst = firstName || (payload.name ? payload.name.split(' ')[0] : 'New');
            const resolvedLast = lastName || (payload.name ? payload.name.split(' ').slice(1).join(' ') : 'User');
            ...
            if (company?.name) {
                addAuditLog(newUser.id!, 'USER_REGISTERED', { type: 'Company', id: companyId, name: company.name });
                addAuditLog(newUser.id!, 'COMPANY_CREATED', { type: 'Company', id: companyId, name: company.name });
            }
...
const calculateToolUsage = (companyId: string, activeUsers: number): ToolUsageInsight[] => {
    return TOOL_DEFINITIONS.map(definition => {
        const { usageCount, lastUsedAt } = definition.resolver(companyId);
        const denominator = Math.max(activeUsers, 1);
        const ratio = usageCount === 0 && activeUsers === 0 ? 0 : usageCount / denominator;
        const adoption = usageCount === 0 && activeUsers === 0 ? 'LOW' : adoptionFromRatio(Math.min(ratio, 1));
        return {
            id: definition.id,
            ...
            lastUsedAt,
            adoption,
            recommendedAction: adoption === 'LOW' ? definition.recommendation : undefined,
        };
    });
};

const calculateAutomationCoverage = (companyId: string) => {
    const todos = getTodosForCompany(companyId);
    if (todos.length === 0) {
        return 0;
    }
...
const calculatePastDueTimesheets = (companyId: string) => {
    const entries = getTimeEntriesForCompany(companyId);
    return entries.filter(entry => {
        if (entry.status && entry.status !== TimesheetStatus.PENDING) {
            return false;
        }
        const reference = entry.endTime ?? entry.clockOut ?? entry.startTime ?? entry.clockIn;
        if (!reference) {
            return false;
        }
        ...
        return Date.now() - date.getTime() > daysToMs(5);
    }).length;
};

const buildTenantOperationalSummary = (companyId: string): TenantOperationalSummary => {
    ...
    const weeklyActiveUsers = users.filter(user => {
        ...
        return isWithinLastDays(user.lastLogin, 7);
    }).length;
    const mfaEnabledUsers = users.filter(user => user.mfaEnabled).length;
    const openIncidents = db.safetyIncidents.filter(incident => {
        if (incident.status === IncidentStatus.RESOLVED) {
            return false;
        }
        ...
        const project = db.projects.find(p => p.id === incident.projectId);
        return project?.companyId === companyId;
    }).length;
    const pastDueTimesheets = calculatePastDueTimesheets(companyId);
    const automationCoverage = calculateAutomationCoverage(companyId);
    const toolUsage = calculateToolUsage(companyId, activeUsers);

    const adoptionScore = (() => {
        const userRatio = activeUsers === 0 ? 0 : Math.min(weeklyActiveUsers / activeUsers, 1);
        const toolScore = toolUsage.length === 0
            ? 0
            : toolUsage.reduce((sum, tool) => {
                  if (tool.adoption === 'HIGH') {
                      return sum + 1;
                  }
                  if (tool.adoption === 'MEDIUM') {
                      return sum + 0.6;
                  }
                  return sum + 0.3;
              }, 0) / toolUsage.length;
        const compliancePenalty = Math.min(
            (snapshot.pendingInvites + pastDueTimesheets + openIncidents) / Math.max(activeUsers, 1),
            1
        );
        const complianceScore = 1 - compliancePenalty;
        const weighted = userRatio * 0.4 + toolScore * 0.4 + complianceScore * 0.2;
        return Math.round(Math.max(0, Math.min(100, weighted * 100)));
    })();

    const recommendedActions = new Set<string>();
    if (weeklyActiveUsers < Math.max(1, Math.ceil(activeUsers * 0.6))) {
        recommendedActions.add('Increase weekly active usage by scheduling workspace walkthroughs with tenant champions.');
    }
    if (mfaEnabledUsers < activeUsers) {
        recommendedActions.add('Encourage multi-factor authentication for all active users to harden security posture.');
    }
    if (pastDueTimesheets > 0) {
        recommendedActions.add('Resolve pending timesheets to keep labour reporting current.');
    }
    if (openIncidents > 0) {
        recommendedActions.add('Close out open safety incidents and share learnings across teams.');
    }
    toolUsage
        .filter(tool => tool.recommendedAction)
        .forEach(tool => recommendedActions.add(tool.recommendedAction!));

    return {
        tenant: snapshot,
        usageScore: adoptionScore,
        utilization: {
            activeUsers,
            weeklyActiveUsers,
            automationCoverage,
...
const MODULE_DEFINITIONS: Array<{
...
}> = [
    {
        id: 'projects',
        label: 'Projects & Map',
        requiredPermissions: [Permission.VIEW_ALL_PROJECTS, Permission.VIEW_ASSIGNED_PROJECTS],
        views: ['projects', 'map'],
        recommendation: 'Assign the user to a project or elevate access so they can collaborate across teams.',
    },
    {
        id: 'tasks',
        label: 'Task Management',
        requiredPermissions: [Permission.VIEW_ALL_TASKS, Permission.MANAGE_ALL_TASKS],
        views: ['all-tasks'],
        recommendation: 'Grant task permissions so the user can coordinate daily work execution.',
    },
    {
        id: 'timesheets',
        label: 'Timesheets',
        requiredPermissions: [Permission.SUBMIT_TIMESHEET, Permission.MANAGE_TIMESHEETS],
        views: ['time', 'timesheets'],
        recommendation: 'Enable timesheet access to keep labour and payroll data current.',
    },
    {
        id: 'documents',
        label: 'Document Control',
        requiredPermissions: [Permission.VIEW_DOCUMENTS, Permission.UPLOAD_DOCUMENTS],
        views: ['documents'],
        recommendation: 'Allow document access so the user can review and upload project files.',
    },
    {
        id: 'safety',
        label: 'Safety Management',
        requiredPermissions: [Permission.VIEW_SAFETY_REPORTS, Permission.MANAGE_SAFETY_REPORTS],
        views: ['safety'],
        recommendation: 'Delegate safety permissions to improve on-site compliance reporting.',
    },
    {
        id: 'financials',
        label: 'Financials & Clients',
        requiredPermissions: [Permission.VIEW_FINANCES, Permission.MANAGE_FINANCES],
        views: ['financials', 'invoices', 'clients'],
        recommendation: 'Provide financial access for stakeholders who manage invoicing and cost control.',
    },
    {
        id: 'team',
        label: 'People & Equipment',
        requiredPermissions: [Permission.VIEW_TEAM, Permission.MANAGE_TEAM, Permission.MANAGE_EQUIPMENT],
        views: ['users', 'equipment'],
        recommendation: 'Grant team and equipment permissions to coordinate resources effectively.',
    },
    {
        id: 'chat',
        label: 'Team Chat',
        requiredPermissions: [Permission.SEND_DIRECT_MESSAGE],
        views: ['chat'],
        recommendation: 'Allow messaging so the user can collaborate through integrated chat.',
    },
    {
        id: 'tools',
        label: 'AI & Tools',
        requiredPermissions: [Permission.ACCESS_ALL_TOOLS],
        views: ['tools'],
        recommendation: 'Provide access to AI assistants and tools to unlock automation benefits.',
    },
    {
        id: 'templates',
        label: 'Templates',
        ...
        views: ['templates'],
        recommendation: 'Enable template access so the user can standardise delivery playbooks.',
    },
    {
        id: 'audit',
        label: 'Audit Log',
        ...
        views: ['audit-log'],
        recommendation: 'Provide audit visibility for governance and change tracking.',
    },
];

const buildUserAccessSummary = (user: Partial<User>, companyId?: string): UserAccessSummary => {
    if (!user.id || !user.role) {
        throw new Error('User not found');
    }
    const resolvedCompanyId = companyId ?? user.companyId ?? null;
    const permissionSet = RolePermissions[user.role] ?? new Set<Permission>();
    const modules: ModuleAccessDescriptor[] = MODULE_DEFINITIONS.map(definition => {
        const hasAny = definition.requiredPermissions.some(permission => permissionSet.has(permission));
        const missing = definition.requiredPermissions.filter(permission => !permissionSet.has(permission));
        return {
            id: definition.id,
            label: definition.label,
            granted: hasAny,
            view: hasAny ? definition.views[0] : undefined,
            reason: hasAny ? undefined : `Missing permissions: ${missing.join(', ')}`,
        };
    });
    ...
    modules.forEach(module => {
        ...
        const definition = MODULE_DEFINITIONS.find(def => def.id === module.id);
        definition?.views.forEach(view => availableViews.add(view));
    });

    const deniedModules: AccessGap[] = modules
    ...
    const recommendedActions = new Set<string>();
    deniedModules.forEach(gap => recommendedActions.add(gap.recommendation));
    if (permissionSet.size === 0) {
        recommendedActions.add('Assign a role or custom permissions so the user can access the platform.');
    }
    ...
    return {
        ...
        availableViews: Array.from(availableViews),
        modules,
        deniedModules,
        recommendedActions: Array.from(recommendedActions),
    };
...
const buildPlatformAdoptionReport = (): PlatformAdoptionReport => {
    const snapshots = getTenantDirectorySnapshots();
    const tenantSummaries = snapshots.map(snapshot => buildTenantOperationalSummary(snapshot.companyId));
    const adoptionScore = tenantSummaries.length === 0
        ? 0
        : Math.round(
              tenantSummaries.reduce((sum, summary) => sum + summary.usageScore, 0) / tenantSummaries.length
          );
    const riskScore = tenantSummaries.length === 0
        ? 0
        : Math.round(
              tenantSummaries.reduce((sum, summary) => {
                  const compliance = summary.compliance;
                  const tenantRisk = compliance.openIncidents + compliance.pastDueTimesheets + compliance.pendingInvites;
                  return sum + tenantRisk;
              }, 0) / tenantSummaries.length
          );
...
export const api = {
    ...
    getTenantOperationalSummary: async (
        companyId: string,
        ...
        await delay();
        ensureNotAborted(options?.signal);
        return buildTenantOperationalSummary(companyId);
    },
    ...
    getUserAccessSummary: async (
        ...
        return buildUserAccessSummary(user, companyId);
    },
    ...
    getPlatformAdoptionReport: async (options?: RequestOptions): Promise<PlatformAdoptionReport> => {
        ensureNotAborted(options?.signal);
        await delay();
        ensureNotAborted(options?.signal);
        return buildPlatformAdoptionReport();
    },
