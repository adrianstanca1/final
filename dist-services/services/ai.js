...
import { TodoStatus, IncidentSeverity, InvoiceStatus, ExpenseStatus, } from '../types.js';
...
const callGemini = async (prompt, overrides = {}) => {
...
const formatCurrency = (value, currency = 'GBP') => new Intl.NumberFormat('en-GB', { style: 'currency', currency, maximumFractionDigits: 0 }).format(value);
...
const inferHealthStatus = (progress, budgetUtilisation, openIncidents) => {
...
const buildTaskSummary = (tasks) => {
    ...
    const averageProgress = total === 0
        ? 0
        : Math.round(tasks.reduce((acc, task) => acc + (task.progress ?? (task.status === TodoStatus.DONE ? 100 : 0)), 0) / total);
...
const buildProjectContext = ({ project, tasks, incidents, expenses }) => {
...
const buildFallbackProjectSummary = (input) => {
...
export const generateProjectHealthSummary = async (input) => {
...
const buildSearchContext = (input) => {
...
const buildFallbackSearch = (input) => {
    const { query, tasks = [], documents = [], incidents = [], expenses = [] } = input;
    const needle = query.toLowerCase();
    const matchedTasks = tasks.filter(task => [task.title, task.text, task.description]
        .filter(Boolean)
        .some(field => field.toLowerCase().includes(needle)));
    const matchedDocuments = documents.filter(doc => [doc.name, doc.category, ...(doc.tags ?? [])]
        .filter(Boolean)
        .some(field => field.toLowerCase().includes(needle)));
    const matchedIncidents = incidents.filter(incident => [incident.title, incident.description]
        .filter(Boolean)
        .some(field => field.toLowerCase().includes(needle)));
    const matchedExpenses = expenses.filter(expense => [expense.description, expense.category]
        .filter(Boolean)
        .some(field => field.toLowerCase().includes(needle)));
    const parts = [`AI search results for "${query}":`];
...
export const searchKnowledgeBase = async (input) => {
...
const computeFinancialSnapshot = (input, horizonMonths) => {
...
const buildForecastContext = (input, snapshot) => {
...
const buildFallbackFinancialForecast = (input, snapshot) => {
