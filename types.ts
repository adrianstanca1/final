...
export type AdoptionLevel = 'HIGH' | 'MEDIUM' | 'LOW';
...
export interface TenantOperationalSummary {
  ...
  utilization: {
    ...
    toolUsage: ToolUsageInsight[];
  };
  compliance: {
    mfaEnabledUsers: number;
    pendingInvites: number;
    pastDueTimesheets: number;
...
export interface UserAccessSummary {
  ...
  permissions: Permission[];
  availableViews: View[];
  modules: ModuleAccessDescriptor[];
  deniedModules: AccessGap[];
  recommendedActions: string[];
}

export interface PlatformAdoptionReport {
  generatedAt: string;
  adoptionScore: number;
  riskScore: number;
  tenants: TenantOperationalSummary[];
  topActions: string[];
}
