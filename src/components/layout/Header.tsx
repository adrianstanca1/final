...
export const Header: React.FC<HeaderProps> = ({ user, onLogout, onSearchClick, onCommandPaletteClick, isOnline = true, unreadNotificationCount, notifications, onNotificationClick, onMarkAllNotificationsAsRead, addToast }) => {
    ...
    return (
        <header className="sticky top-0 z-30 bg-background/80 backdrop-blur-lg border-b border-border h-16 flex-shrink-0 flex items-center justify-end px-6 gap-4">
            {!isOnline && (
                <div className="mr-auto hidden md:flex items-center gap-2 text-xs text-amber-600" aria-label="Offline mode">
                    <span className="h-2 w-2 rounded-full bg-amber-500 animate-pulse" />
                    <span>Offline mode</span>
                </div>
            )}
            <button onClick={onSearchClick} className="p-2 rounded-full hover:bg-accent text-muted-foreground" aria-label="Search">
            ...
            <div className="relative">
                <button onClick={() => setIsNotificationMenuOpen(prev => !prev)} className="p-2 rounded-full hover:bg-accent text-muted-foreground relative" aria-label="Notifications">
