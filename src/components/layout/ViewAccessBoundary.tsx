        ...
        <div key={`${group.join('-')}-${index}`} className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            {group.map((permission) => (
              <Tag key={permission} label={humanise(String(permission))} color="yellow" statusIndicator="yellow" />
            ))}
          </div>
        </div>
      ))}
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                {access.reason ||
                  `This workspace area is restricted for your current role. ${description ? `It covers: ${description}` : ''
                  }`}
              </p>
            </div>
          </div>
