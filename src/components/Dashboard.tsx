...
export const Dashboard: React.FC<DashboardProps> = ({ user, addToast, setActiveView, onSelectProject }) => {
    ...
    useEffect(() => {
        ...
        if (projectList.length === 0) {
            setAiSelectedProjectId(prev => prev && projectList.some(project => project.id === prev) ? prev : null);
            return;
        }

        setAiSelectedProjectId(prev => {
            if (prev && projectList.some(project => project.id === prev)) {
                return prev;
            }
            const activeProject = projectList.find(project => (project.status ?? '').toString().toUpperCase() === 'ACTIVE');
            return activeProject?.id ?? projectList[0]?.id ?? null;
        });
    }, [snapshot?.projects]);
    ...
    const operationalInsights = snapshot?.operationalInsights ?? null;
    const metadata = snapshot?.metadata;

    const userMap = useMemo(() => new Map(team.map(u => [u.id, u])), [team]);

    const portfolioSummary: ProjectPortfolioSummary = useMemo(
        () => (snapshot?.portfolioSummary ?? computeProjectPortfolioSummary(projects)),
        [snapshot, projects],
    );
