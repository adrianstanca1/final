  ...
  return (
    <Card>
      ...
      <p className="text-sm text-muted-foreground mb-4">
        The following actions failed to sync with the server. You can retry them or discard them if they are no longer needed.
      </p>
        ...
        {failedActions.map(action => (
          <div key={action.id} className="p-3 border rounded-md bg-destructive/5 dark:bg-destructive/10">
            ...
            <div className="flex justify-end gap-2 mt-2">
              <Button size="sm" variant="secondary" onClick={() => handleDiscard(action.id)} disabled={isLoading}>
                Discard
              </Button>
              <Button size="sm" variant="primary" onClick={() => handleRetry(action.id)} isLoading={isLoading}>
                Retry
              </Button>
            </div>
          </div>
    ...
    <Card>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="flex items-center gap-6">
          ...
          />
          <div>
            <Button type="button" variant="secondary" onClick={handleAvatarUpload}>
              Upload Photo
            </Button>
...
const CompanySettingsComponent: React.FC<{
  ...
  const handleSettingsChange = (key: keyof CompanySettings, value: CompanySettings[typeof key]) => {
    if (settings[key] === value || isPending(String(key))) {
      return;
    }

    let successMessage = 'Company settings updated.';
    if (key === 'theme') {
      const themeMode = value === 'dark' ? 'dark' : 'light';
      successMessage = `Theme set to ${themeMode} mode.`;
    }
