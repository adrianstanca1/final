...
const CreateCompanyModal: React.FC<{
    ...
    const handleTypeChange = (_: string, val: string) => {
        ...
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
            ...
            <Card className="w-full max-w-lg relative">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <SelectField label="Company Type" name="type" value={type} onChange={(_, val) => handleTypeChange(_, val)} error={errors.type} options={companyTypeOptions} />
...
const SelectField = ({ label, name, value, onChange, error, options }: { label: string, name: string, value: any, onChange: (name: string, value: string) => void, error?: string, options: { value: string, label: string }[] }) => (
    <div>
...
export const UserRegistration: React.FC<UserRegistrationProps> = ({ onSwitchToLogin }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            // Prepare the registration data based on company selection
            const registrationData: Partial<RegisterCredentials> = {
            ...
            // Add company information if creating a company
            if (formData.companySelection === 'create' && formData.companyName) {
                registrationData.companyName = formData.companyName;
                registrationData.companyType = formData.companyType;
                registrationData.companyEmail = formData.companyEmail;
                registrationData.companyPhone = formData.companyPhone;
                registrationData.companyWebsite = formData.companyWebsite;
            } else if (formData.companySelection === 'join' && formData.inviteToken) {
                registrationData.inviteToken = formData.inviteToken;
            }

            await register(registrationData);
            // On success, the AuthProvider will handle navigation.
        } catch (error: any) {
            // Error might be handled by the context and set via useEffect
                        ...
                        <Card className="bg-muted">
                            ...
                            {formData.role ? (
                                <ul className="text-sm space-y-1 list-disc list-inside max-h-60 overflow-y-auto">
                                    {Array.from(selectedRolePermissions).map((p: any) => <li key={String(p)} className="capitalize">{String(p).replace(/_/g, ' ').toLowerCase()}</li>)}
                                </ul>
            ...
            case 'terms': return (
                <div>
                    <div className="flex items-start">
                        ...
                        <label htmlFor="terms" className="ml-2 block text-sm text-muted-foreground">I agree to the <button type="button" onClick={() => window.alert('Terms and Conditions clicked')} className="font-medium text-primary hover:text-primary/90">Terms and Conditions</button> and <button type="button" onClick={() => window.alert('Privacy Policy clicked')} className="font-medium text-primary hover:text-primary/90">Privacy Policy</button>.</label>
                    </div>
    ...
    const RadioCard = ({ name, value, label, description, checked, onChange }: { name: string, value: string, label: string, description: string, checked: boolean, onChange: (name: string, value: string) => void }) => (
