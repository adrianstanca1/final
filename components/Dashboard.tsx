...
export const Dashboard: React.FC<DashboardProps> = ({ user, addToast, setActiveView, onSelectProject }) => {
    ...
    const workforceTrend = insight?.trends.workforce ?? [];
                ...
                <Card className="space-y-4 p-6">
                    ...
                    {workforceTrend.length > 0 && (
                        <div className="mt-4 space-y-2">
                            ...
                            {workforceComplianceDelta !== null && (
                                <p
                                    className={`text-[11px] font-medium ${
                                        workforceComplianceDelta >= 0 ? 'text-emerald-600' : 'text-destructive'
                                    }`}
                                >
                                    {workforceComplianceDelta >= 0 ? '▲' : '▼'}{' '}
                                    {Math.abs(workforceComplianceDelta).toFixed(1)} pts vs last week
                                </p>
                            )}
                        </div>
                    )}
