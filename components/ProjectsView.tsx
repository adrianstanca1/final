...
import { User, Project, Todo, ProjectAssignment, Permission, TodoStatus, ProjectHealth, Role } from '../types';
...
export const ProjectsView: React.FC<ProjectsViewProps> = ({ user, addToast, onSelectProject }) => {
    ...
    const pmKpis = useMemo(() => {
        ...
        const totalActualCost = allProjects.reduce((sum, p) => sum + p.actualCost, 0);
        ...
        allProjects.forEach(p => {
            ...
            if (stats) {
                totalOpenTasks += stats.taskCount;
                totalOverdueTasks += stats.overdueTaskCount;
            }
        });
        
        const totalTasks = totalOpenTasks + totalOverdueTasks;
        const overduePercentage = totalTasks > 0 ? (totalOverdueTasks / totalTasks) * 100 : 0;
    ...
    }, [isPM, allProjects, allAssignments, projectStats]);
    ...
    return (
        <div className="space-y-6">
            ...
            {isPM && pmKpis && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-card-enter">
                    <KpiCard title="Portfolio Budget">
                        <span className="text-green-600">{formatCurrency(pmKpis.totalActualCost)}</span>
                        <span className="text-lg text-slate-400 font-medium"> / {formatCurrency(pmKpis.totalBudget)}</span>
                    </KpiCard>
                     <KpiCard title="Overdue Tasks">
                        <span className={pmKpis.overduePercentage > 10 ? 'text-red-600' : 'text-slate-800'}>
                            {pmKpis.overduePercentage.toFixed(1)}%
                        </span>
                    </KpiCard>
                    <KpiCard title="Total Team Members">
                        {pmKpis.totalTeamMembers}
                    </KpiCard>
                </div>
            )}
