// full contents of components/layout/Sidebar.tsx

import React from 'react';
// FIX: Corrected import paths to be relative.
// FIX: Imported View and Role enums.
...
export const Sidebar: React.FC<SidebarProps> = ({ user, activeView, setActiveView, onLogout, pendingTimesheetCount, openIncidentCount, unreadMessageCount }) => {
        ...
        return (
            <>
                <NavSection title="Overview">
                    ...
                    {/* FIX: Used Permission enum for hasPermission checks */}
                    {hasPermission(user, Permission.SEND_DIRECT_MESSAGE) && <NavItem view='chat' label='Chat' icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg>} activeView={activeView} setActiveView={setActiveView} badgeCount={unreadMessageCount} />}
                </NavSection>

                 <NavSection title="Management">
                    {(hasPermission(user, Permission.VIEW_ASSIGNED_PROJECTS) || hasPermission(user, Permission.VIEW_ALL_PROJECTS)) && <NavItem view='projects' label='Projects' icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 10h16M4 14h16M4 18h16" /></svg>} activeView={activeView} setActiveView={setActiveView} />}
                    {hasPermission(user, Permission.VIEW_ALL_TASKS) && <NavItem view='all-tasks' label='All Tasks' icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" /></svg>} activeView={activeView} setActiveView={setActiveView} />}
                    {hasPermission(user, Permission.VIEW_ASSIGNED_PROJECTS) && <NavItem view='map' label='Map View' icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l5.447 2.724A1 1 0 0021 16.382V5.618a1 1 0 00-1.447-.894L15 7m-6 3l6-3m0 0l6 3m-6-3v10" /></svg>} activeView={activeView} setActiveView={setActiveView} />}
                    ...
                    {(hasPermission(user, Permission.MANAGE_TIMESHEETS) || hasPermission(user, Permission.VIEW_ALL_TIMESHEETS)) && <NavItem view='timesheets' label='Timesheets' icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" /></svg>} activeView={activeView} setActiveView={setActiveView} badgeCount={pendingTimesheetCount} />}
                    ...
                    {hasPermission(user, Permission.MANAGE_EQUIPMENT) && <NavItem view='equipment' label='Equipment' icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.196-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.783-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" /></svg>} activeView={activeView} setActiveView={setActiveView} />}
                    {hasPermission(user, Permission.MANAGE_PROJECT_TEMPLATES) && <NavItem view='templates' label='Templates' icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" /></svg>} activeView={activeView} setActiveView={setActiveView} />}
                </NavSection>

                <NavSection title="Tools">
                    {hasPermission(user, Permission.ACCESS_ALL_TOOLS) && <NavItem view='tools' label='AI Tools' icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6V4m0 2a2 2 0 100 4m0-4a2 2 0 110 4m-6 8a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4m6 6v10m6-2a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4" /></svg>} activeView={activeView} setActiveView={setActiveView} />}
                    {hasPermission(user, Permission.VIEW_AUDIT_LOG) && <NavItem view='audit-log' label='Audit Log' icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>} activeView={activeView} setActiveView={setActiveView} />}
                </NavSection>
            </>
        );
    }
    ...
    return (
        <aside className="w-64 bg-muted p-4 flex flex-col flex-shrink-0 h-screen">
            ...
            <div className="mt-auto">
                ...
                <button
                    onClick={onLogout}
                    className="w-full flex items-center gap-3 px-4 py-2.5 rounded-md text-sm text-muted-foreground hover:bg-accent hover:text-accent-foreground mt-2"
                >
