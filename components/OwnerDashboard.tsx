...
export const OwnerDashboard: React.FC<OwnerDashboardProps> = ({
  ...
  const workforceTrend = operationalInsights?.trends.workforce ?? [];
  const financialTrend = operationalInsights?.trends.financial ?? [];
  const safetyTrend = operationalInsights?.trends.safety ?? [];
  ...
  const complianceDelta =
    latestWorkforcePoint && previousWorkforcePoint
      ? Math.round((latestWorkforcePoint.complianceRate - previousWorkforcePoint.complianceRate) * 10) / 10
      : null;

  const latestFinancialPoint = financialTrend.length ? financialTrend[financialTrend.length - 1] : null;
  const previousFinancialPoint = financialTrend.length > 1 ? financialTrend[financialTrend.length - 2] : null;
  const approvedExpenseDelta =
    latestFinancialPoint && previousFinancialPoint
      ? latestFinancialPoint.approvedExpenses - previousFinancialPoint.approvedExpenses
      : null;
  const outstandingDelta =
    latestFinancialPoint && previousFinancialPoint
      ? latestFinancialPoint.outstandingReceivables - previousFinancialPoint.outstandingReceivables
      : null;

  const latestSafetyPoint = safetyTrend.length ? safetyTrend[safetyTrend.length - 1] : null;
  const previousSafetyPoint = safetyTrend.length > 1 ? safetyTrend[safetyTrend.length - 2] : null;
  const incidentDelta =
    latestSafetyPoint && previousSafetyPoint
      ? latestSafetyPoint.incidents - previousSafetyPoint.incidents
      : null;

  const workforceChartData = workforceTrend.map(point => ({
    label: point.periodLabel,
    value: Math.max(0, point.complianceRate),
  }));
  const financialChartData = financialTrend.map(point => ({
    label: point.periodLabel,
    value: Math.max(0, point.approvedExpenses),
  }));
  const safetyChartData = safetyTrend.map(point => ({
    label: point.periodLabel,
    value: Math.max(0, point.incidents),
  }));
  const recentSafetyTrend = safetyTrend.slice(-3).reverse();

  const complianceDeltaClass =
    complianceDelta === null ? 'text-muted-foreground' : complianceDelta >= 0 ? 'text-emerald-600' : 'text-destructive';
  const complianceDeltaText =
    complianceDelta === null
      ? 'No prior week data'
      : `${complianceDelta >= 0 ? '▲' : '▼'} ${Math.abs(complianceDelta).toFixed(1)} pts vs last week`;

  const approvedExpenseDeltaClass =
    approvedExpenseDelta === null
      ? 'text-muted-foreground'
      : approvedExpenseDelta > 0
      ? 'text-amber-600'
      : 'text-emerald-600';
  const approvedExpenseDeltaText =
    approvedExpenseDelta === null
      ? 'No prior month data'
      : `${approvedExpenseDelta > 0 ? '▲' : '▼'} ${formatCurrency(Math.abs(approvedExpenseDelta), financialCurrency)} vs prior month`;

  const outstandingDeltaClass =
    outstandingDelta === null
      ? 'text-muted-foreground'
      : outstandingDelta > 0
      ? 'text-destructive'
      : 'text-emerald-600';
  const outstandingDeltaText =
    outstandingDelta === null
      ? 'No prior month data'
      : `${outstandingDelta > 0 ? '▲' : '▼'} ${formatCurrency(Math.abs(outstandingDelta), financialCurrency)} vs prior month`;

  const incidentDeltaClass =
    incidentDelta === null
      ? 'text-muted-foreground'
      : incidentDelta > 0
      ? 'text-destructive'
      : 'text-emerald-600';
  const incidentDeltaText =
    incidentDelta === null
      ? 'No prior week data'
      : `${incidentDelta > 0 ? '▲' : '▼'} ${Math.abs(incidentDelta)} vs last week`;
      ...
      <section className="grid gap-6 lg:grid-cols-3">
        <Card className="space-y-4 p-6">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h2 className="text-lg font-semibold text-foreground">Workforce rhythm</h2>
              <p className="text-sm text-muted-foreground">Six-week approval trend for field teams.</p>
            </div>
            <span className={`text-xs font-semibold ${complianceDeltaClass}`}>{complianceDeltaText}</span>
          </div>
          <MiniBarChart
            data={workforceChartData}
            accent="emerald"
            emptyLabel="Submit timesheets to build compliance history."
          />
          {latestWorkforcePoint ? (
            <div className="grid grid-cols-3 gap-3 text-xs text-muted-foreground">
              <div>
                <p>Latest compliance</p>
                <p className="font-semibold text-foreground">{latestWorkforcePoint.complianceRate.toFixed(1)}%</p>
              </div>
              <div>
                <p>Approved</p>
                <p className="font-semibold text-foreground">{latestWorkforcePoint.approved}</p>
              </div>
              <div>
                <p>Pending</p>
                <p className="font-semibold text-foreground">{latestWorkforcePoint.pending}</p>
              </div>
            </div>
          ) : (
            <p className="text-sm text-muted-foreground">No submitted timesheets in the last six weeks.</p>
          )}
        </Card>
        ...
        <Card className="space-y-4 p-6">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h2 className="text-lg font-semibold text-foreground">Cost velocity</h2>
              <p className="text-sm text-muted-foreground">Approved spend and open receivables.</p>
            </div>
            <span className={`text-xs font-semibold ${approvedExpenseDeltaClass}`}>{approvedExpenseDeltaText}</span>
          </div>
          <MiniBarChart
            data={financialChartData}
            accent="amber"
            emptyLabel="Record expenses to build cost history."
          />
            ...
            <div className="grid grid-cols-2 gap-4 text-xs text-muted-foreground">
              <div>
                ...
                <p className="font-semibold text-foreground">
                  {formatCurrency(latestFinancialPoint.approvedExpenses, financialCurrency)}
                </p>
                <p className={`text-[11px] ${approvedExpenseDeltaClass}`}>{approvedExpenseDeltaText}</p>
              </div>
              <div>
                <p>Outstanding due</p>
                <p className="font-semibold text-foreground">
                  {formatCurrency(latestFinancialPoint.outstandingReceivables, financialCurrency)}
                </p>
                <p className={`text-[11px] ${outstandingDeltaClass}`}>{outstandingDeltaText}</p>
              </div>
            </div>
          ) : (
            <p className="text-sm text-muted-foreground">No approved costs recorded in the last six months.</p>
          )}
        </Card>
        ...
        <Card className="space-y-4 p-6">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h2 className="text-lg font-semibold text-foreground">Safety climate</h2>
              <p className="text-sm text-muted-foreground">Incident frequency and severity trend.</p>
            </div>
            <span className={`text-xs font-semibold ${incidentDeltaClass}`}>{incidentDeltaText}</span>
          </div>
          <MiniBarChart
            data={safetyChartData}
            accent="primary"
            emptyLabel="Log incidents to monitor safety trends."
          />
          {latestSafetyPoint ? (
            <div className="space-y-3 text-xs text-muted-foreground">
              <div className="flex items-center justify-between">
                ...
                <span className="font-semibold text-foreground">
                  {latestSafetyPoint.incidents} incident{latestSafetyPoint.incidents === 1 ? '' : 's'}
                  {latestSafetyPoint.highSeverity > 0 ? ` • ${latestSafetyPoint.highSeverity} high` : ''}
                </span>
              </div>
              <ul className="space-y-2">
                {recentSafetyTrend.map(point => (
                  <li key={point.periodStart} className="flex items-center justify-between">
                    <span>{point.periodLabel}</span>
                    <span className="font-semibold text-foreground">
                      {point.incidents}
                      {point.highSeverity > 0 ? ` (${point.highSeverity} high)` : ''}
                    </span>
                  </li>
                ))}
              </ul>
            ...
            <p className="text-sm text-muted-foreground">No incident reports captured in the last six weeks.</p>
          )}
        </Card>
      </section>
