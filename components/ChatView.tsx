                ...
                <div className="space-y-2 max-h-96 overflow-y-auto">
                    {availableUsers.length > 0 ? availableUsers.map(p => (
                        <button key={p.id} onClick={() => onStartConversation(p)} className="w-full flex items-center gap-3 p-2 rounded-lg hover:bg-slate-100">
                            <Avatar name={p.name} imageUrl={p.avatarUrl} className="w-10 h-10"/>
                            <div>{p.name}</div>
                        </button>
                        ...
                        {conversations.map(convo => {
                            ...
                            return (
                                <button key={convo.id} onClick={() => setActiveConversationId(convo.id)} className={`w-full text-left p-4 flex gap-3 items-center relative ${activeConversationId === convo.id ? 'bg-sky-50 dark:bg-sky-900/50' : 'hover:bg-slate-50 dark:hover:bg-slate-800'}`}>
                                    ...
                                    <Avatar name={otherUser.name} imageUrl={otherUser.avatarUrl} className="w-10 h-10" />
                                    <div className="flex-grow overflow-hidden">
                                        <p className={`font-semibold ${isUnread ? 'text-slate-900 dark:text-slate-50' : ''}`}>{otherUser.name}</p>
                    ...
                    {activeConversation ? (
                        <>
                            <div className="p-4 border-b dark:border-slate-700 flex items-center gap-3">
                                <Avatar name={getOtherParticipant(activeConversation)?.name || ''} imageUrl={getOtherParticipant(activeConversation)?.avatarUrl} className="w-8 h-8" />
                            ...
                            <div className="flex-grow p-4 overflow-y-auto space-y-4">
                                {messages.map(msg => (
                                    <div key={msg.id} className={`flex gap-3 ${msg.senderId === user.id ? 'justify-end' : 'justify-start'}`}>
                                        {msg.senderId !== user.id && <Avatar name={getOtherParticipant(activeConversation)?.name || ''} imageUrl={getOtherParticipant(activeConversation)?.avatarUrl} className="w-8 h-8 self-end"/>}
