...
export const Login: React.FC<LoginProps> = ({ onSwitchToRegister, onSwitchToForgotPassword }) => {
    const { login, loginWithGoogle, verifyMfaAndFinalize, error: authError, loading: isLoading } = useAuth();
    ...
    const handleGoogleLogin = async (response: CredentialResponse) => {
        ...
        try {
            ...
            persistRememberedEmail(rememberMe, session.user.email.toLowerCase());
        } catch (err: any) {
        ...
        {googleEnabled && (
            <>
                <div className="relative flex items-center justify-center">
                    <span className="absolute bg-card px-2 text-xs font-medium uppercase tracking-wider text-muted-foreground">or</span>
                    <div className="w-full border-t border-border" />
                </div>
                <div className="flex flex-col items-center gap-2">
                    <div style={{ pointerEvents: isGoogleLoading ? 'none' : undefined, opacity: isGoogleLoading ? 0.6 : 1 }}>
                        <GoogleLogin
                    ...
                    {isGoogleLoading && (
                        <p className="text-xs text-muted-foreground">Connecting to Google…</p>
                    )}
                </div>
            </>
        )}
