...
import { User, Project, Task as Todo, TodoPriority } from '../types';
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select id="project-select" value={projectId} onChange={e => setProjectId(e.target.value)} className="w-full p-2 border rounded bg-white" required>
                                {projects.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                            </select>
                        </div>
                        <div>
                            <label htmlFor="assignee-select" className="block text-sm font-medium">Assignee</label>
                            <select id="assignee-select" value={assigneeId} onChange={e => setAssigneeId(e.target.value)} className="w-full p-2 border rounded bg-white">
                                <option value="">Unassigned</option>
                                {users.map(u => <option key={u.id} value={u.id}>{`${u.firstName} ${u.lastName}`}</option>)}
                            </select>
                        </div>
                        <div>
                            <label htmlFor="due-date-input" className="block text-sm font-medium">Due Date</label>
                            <input id="due-date-input" type="date" value={dueDate} onChange={e => setDueDate(e.target.value)} className="w-full p-2 border rounded" />
                        </div>
                        <div>
                            <label htmlFor="priority-select" className="block text-sm font-medium">Priority</label>
                            <select id="priority-select" value={priority} onChange={e => setPriority(e.target.value as TodoPriority)} className="w-full p-2 border rounded bg-white">
                                {Object.values(TodoPriority).map(p => <option key={p} value={p}>{p}</option>)}
                            </select>
                        </div>
                    </div>
                    <div>
                        <label htmlFor="depends-on-select" className="block text-sm font-medium">Depends On (blockers)</label>
                        <select
