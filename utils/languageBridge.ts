...
/**
 * Data format types supported by the bridge
 */
 ...
 */
export async function transformData<T = any>(
  data: any, 
  ...
  if (sourceFormat === 'json') {
    if (targetFormat === 'text') {
      return typeof data === 'string' ? data : JSON.stringify(data) as unknown as T;
    }
    ...
    if (targetFormat === 'base64') {
      ...
      if (typeof btoa !== 'undefined') {
        // Browser
        return btoa(jsonString) as unknown as T;
      } else if (typeof Buffer !== 'undefined') {
        // Node.js
        return Buffer.from(jsonString).toString('base64') as unknown as T;
      }
    }
  }
  ...
  if (sourceFormat === 'text') {
    ...
    if (targetFormat === 'base64') {
      if (typeof btoa !== 'undefined') {
        // Browser
        return btoa(data) as unknown as T;
      } else if (typeof Buffer !== 'undefined') {
        // Node.js
        return Buffer.from(data).toString('base64') as unknown as T;
      }
    }
  }
  ...
  if (sourceFormat === 'base64') {
    if (targetFormat === 'text') {
      if (typeof atob !== 'undefined') {
        // Browser
        return atob(data) as unknown as T;
      } else if (typeof Buffer !== 'undefined') {
        // Node.js
 ...
 */
export function createLanguageBridge<T extends object>(
  targetObject: T,
  sourceFormat: DataFormat,
  targetFormat: DataFormat
): T {
      ...
      if (typeof value === 'function') {
        return async (...args: any[]) => {
          ...
          const transformedArgs = await Promise.all(
            args.map(arg => transformData(arg, {
              sourceFormat,
              targetFormat,
              preserveTypes: true
            }))
 ...
 */
export async function connectToPythonService<T extends object>(
  serviceUrl: string,
  ...
  return Object.keys(interfaceDefinition).reduce((acc, key) => {
    const method = interfaceDefinition[key as keyof T];
...
export async function processMultimodalImage(
  ...
  prompt: string,
  options: { useLocal?: boolean } = {}
): Promise<{ prompt: string, mimeType: string, data: string }> {
